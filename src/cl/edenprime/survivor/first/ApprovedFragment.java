...
public class ApprovedFragment extends Fragment {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
		return inflater.inflate(R.layout.firsttime3, container, false);
	}
	
	@Override
	public void onActivityCreated(Bundle savedInstanceState) {
		super.onActivityCreated(savedInstanceState);
		content = (LinearLayout) getView().findViewById(R.id.FirstCheckBoxesAp);
		content.removeAllViews();
		
		checkboxRamos = new ArrayList<CheckBox>();
		
		ramos = dataS.getServiciosDatos().obtenerRamos();
	...
	public void saveApprovedRamos() {
		ArrayList<Ramo> aprobados = new ArrayList<Ramo>();
		for (int i = 0; i < checkboxRamos.size(); i++) {
			if (checkboxRamos.get(i).isChecked()) {
				aprobados.add(ramos.get(i));
			}
		}
		dataS.getServiciosDatos().aprobarRamos(aprobados);
	}
